...
POD_INV=$(minikube kubectl -- get pods -l app=servicio-inventario -o jsonpath="{.items[0].metadata.name}" 2>/dev/null)
if [ -z "$POD_INV" ]; then
    echo -e "${RED}Error: No se encontró el pod de servicio-inventario.${NC}"
else
    echo "Eliminando pod de forma abrupta: $POD_INV"
    minikube kubectl -- delete pod $POD_INV --now[cite: 3]
...
minikube kubectl -- set env deployment/servicio-pagos-stub LATENCIA_ACTIVA=true[cite: 3]
...
minikube kubectl -- scale deployment servicio-notificaciones --replicas=0[cite: 3]
...
minikube kubectl -- scale deployment servicio-notificaciones --replicas=1[cite: 3]
echo ""
